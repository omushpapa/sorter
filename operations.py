...
def display_message(text, status):
...
def initiate_operation(src='', dst='', sort=False, recur=False, types=None, status=None, parser=None):
    proceed = True
    ...
    if src:
        ...
        proceed = False
        display_message('Source folder is required.', status=status)
    ...
    if proceed:
        if not os.path.isdir(source_path):
            proceed = False
            display_message(
                'Given source folder is NOT a folder.', status=status)
        else:
    ...
    if proceed:
        if destination_path:
            if not os.path.isdir(destination_path):
                proceed = False
                display_message(
                    'Given destination folder is NOT a folder.', status=status)
            else:
